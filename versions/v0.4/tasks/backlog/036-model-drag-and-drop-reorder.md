...
- [ ] Providers can be reordered by dragging and dropping
- [ ] Visual feedback indicates the dragged item and the drop target position
- [ ] The new order is persisted via `PUT /api/config/models` on drop
- [ ] Position labels update to reflect the new order
- [ ] Drag handles are visible on each provider card
- [ ] Failed saves revert the list to the previous order with an error message
- [ ] Drag-and-drop works on both desktop (mouse) and mobile (touch)

## Test Cases

- [ ] Drag the second provider to the first position; assert `PUT /api/config/models` is called with the reversed order
- [ ] After reorder, assert position labels update (the dragged item shows "Primary")
- [ ] Simulate a save failure after reorder; assert the list reverts to the original order
- [ ] Assert the drag handle is visible on each provider card
- [ ] Drag a provider within a single-item list (no-op); assert no API call is made
- [ ] Reorder chat providers; assert embedding providers are unchanged in the saved config

...
The Settings UI establishes the user-facing configuration patterns that persist through all future versions. When `buddy-core` is extracted in V0.5, the config management API becomes a shared library that the web UI, Telegram bot, and CLI all consume. The drag-and-drop model priority UI is reusable for agent model selection in V0.6.

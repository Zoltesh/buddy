...
- [ ] Network failure during auth check does NOT set `authenticated = true`
- [ ] Network failure shows an error message to the user
- [ ] Error state includes a Retry button that re-checks auth status
- [ ] Successful auth check still works as before (no regression)
- [ ] When server comes back online, clicking Retry recovers to normal login flow
- [ ] All existing tests pass

## Test Cases

- [ ] Mock `fetch('/api/auth/status')` to throw a network error; assert `authenticated` is `false` and `authRequired` is `true`
- [ ] Mock `fetch('/api/auth/status')` to throw; assert `authError` contains an error message string
- [ ] Mock `fetch('/api/auth/status')` to return `{ required: true }`; assert normal login flow is shown (no regression)
- [ ] Mock `fetch('/api/auth/status')` to return `{ required: false }`; assert app loads normally (no regression)

...
- [x] `read_file.rs` no longer has `.unwrap()` on `canonicalize()` — uses `?` with error mapping instead
- [x] The remaining `.expect()` calls have clear justification comments
- [x] All existing tests pass (`cargo test`)

## Test Cases

- [x] Call `ReadFileSkill` with a valid file path; assert it returns success (existing behavior preserved)
- [x] Call `ReadFileSkill` where the allowed directory has been removed between construction and execution; assert it returns a `SkillError`, not a panic

...
- **Interfaces section in navigation:** The application shell's navigation rail (established in V0.4) gains an "Interfaces" section for managing connected channels. Users can view connection status, configure, and toggle Telegram/WhatsApp from the web UI. The collapsible sidebar pattern scales naturally — each interface is a nav item or a sub-section, not a layout change.
...
The clean `buddy-core` separation means agents (V0.6) are just multiple concurrent consumers of the core library. The multi-provider support means agents can use different models for different tasks. The CLI tooling provides the foundation for headless agent orchestration. The navigation shell grows from three items (Chat, Interfaces, Settings) to include agent management in V0.6 without any structural changes to the sidebar layout.
